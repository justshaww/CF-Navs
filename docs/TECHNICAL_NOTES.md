...
后台站点设置中的背景仍使用现有 `backgrounds.light` / `backgrounds.dark` 结构保存。内置「清透蓝绿」和「晨雾石青」只是前端预设：选择后会同时写入浅色/深色 `gradient` 背景、遮罩参数、卡片背景透明度，并把首页标题色与卡片文字色留空以跟随当前主题。用户继续手动修改浅色/深色背景值后，界面会按自定义渐变处理，不需要新增后端字段。
