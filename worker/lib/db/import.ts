...
import { sortBookmarksParentsFirst } from '../../../shared/bookmarkTree'
...
export async function importData(
  ...
  for (const chunk of chunkImportRows(sortBookmarksParentsFirst(importedBookmarks), 7)) {
    stmts.push(db.prepare(`INSERT INTO bookmarks (id, category_id, parent_id, title, url, icon, icon_source, icon_background_color, icon_blob, description, description_mode, open_method, sort, created_at) VALUES ${chunk.map(() => '(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').join(', ')}`)
      .bind(...chunk.flatMap((bookmark) => [bookmark.id, bookmark.category_id, bookmark.parent_id ?? null, bookmark.title, bookmark.url, bookmark.icon, bookmark.icon_source, bookmark.icon_background_color, bookmark.icon_blob, bookmark.description, bookmark.description_mode ?? null, bookmark.open_method, bookmark.sort, bookmark.created_at])))
  }
