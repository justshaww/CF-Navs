...
export async function ensureSchema(db: D1Database, force = false): Promise<void> {
  ...
  if (!colNames.has("parent_id")) {
    stmts.push(db.prepare("ALTER TABLE bookmarks ADD COLUMN parent_id INTEGER"))
  }
  stmts.push(db.prepare("CREATE INDEX IF NOT EXISTS idx_bookmarks_sort_global ON bookmarks(sort, id)"))
  stmts.push(db.prepare("CREATE INDEX IF NOT EXISTS idx_bookmarks_parent_sort ON bookmarks(parent_id, sort, id)"))
